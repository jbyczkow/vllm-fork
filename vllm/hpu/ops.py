...
def static_fused_moe(hidden_states, w1, w2, score, topk):
    ...
    w1_transposed = torch.permute(w1, (0, 2, 1))
    w2_transposed = torch.permute(w2, (0, 2, 1))
    w11_list = [w1_transposed[i,:,:d].squeeze() for i in range(num_experts)]
    w12_list = [w1_transposed[i,:,d:].squeeze() for i in range(num_experts)]
    w2_list = [w2_transposed[i,:,:].squeeze() for i in range(num_experts)]
    ...
    final_hidden_states = torch.ops.hpu.mixture_of_experts(
            ...
            expert_weights_1=w11_list,
            expert_weights_2=w12_list,
            expert_weights_3=w2_list,
            activation="silu",
            experts_min=0,
            experts_max=7
    )
